#!/usr/bin/env python3
"""Run a prompt-enhanced CartPole agent with Jev through TypeSafe's API.

This starts from ``naive_jev_cartpole_agent.py`` and adds qualitative control
guidance: predict several future states, account for velocity and inertia,
correct before boundaries, prioritize imminent pole-angle failure, and avoid
rapid action switching caused by tiny observation changes.
"""
...
def _make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the enhanced CartPole prompt derived from the naive agent "
