#!/usr/bin/env python3
"""Run a minimal CartPole agent with Jev through TypeSafe's official API.

This is the naive prompt baseline from the CartPole task in
https://github.com/taodav/jev_deep_rl/.  It sends the current named
observation, the goal, and only the previous action/reward.  The API response
remains a typed ``noul`` answer so it can be used with the rest of this
project's TypeSafe integration.
"""
...
def make_state(
    ...
    """Build the minimal CartPole state expected by the Jev prompt."""
...
class TypeSafeJev:
    ...
    def choose(self, state: Mapping[str, Any]) -> JevDecision:
        """Ask the CartPole noul question for the next action."""
...
def _make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the minimal CartPole prompt from jev_deep_rl "
